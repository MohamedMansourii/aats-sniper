...
/* -------------------------------------------------------------------------- */
/*  Palette — hardcoded hex (recharts canvas can't read CSS vars).            */
/* -------------------------------------------------------------------------- */
...
export default function Model() {
  const { data: pred, loading: predLoading, error: predError } = usePredictions();
  ...
  const calibBins = pred?.calibrationBins ?? [];
  ...
  const calErr = pred ? round2(calibrationError(pred.calibrationBins) * 100) : 0;
        ...
        {!beatingBaseline && (
          <div className="flex items-start gap-2.5 rounded-[10px] border border-[color:color-mix(in_srgb,var(--danger)_30%,transparent)] bg-[color:color-mix(in_srgb,var(--danger)_10%,transparent)] px-3.5 py-2.5">
            ...
            <div className="text-[11px] leading-relaxed text-text">
              ...
              been <span className="font-semibold text-danger">auto-disabled</span>{" "}
              and the agent has reverted to gate-only sniping until edge recovers.
            </div>
          </div>
        )}
                ...
                {featData.map((f) => (
                  <div key={f.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <Gauge value={f.weight} max={featData[0].weight} tone="info" />
                    </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start">
            <div className="flex-1 space-y-1.5 text-[12px] leading-relaxed text-muted-foreground">
              <p>
                ...
                freshly-launched token both <span className="text-text">lands</span>{" "}
                the entry and reaches the first take-profit before the hard stop,
                conditioned on gate features, on-chain flow and social
                synchronicity.
              </p>
              <p>
              ...
              <p className="text-faint">
                ...
                <span className="font-medium text-text">
                  auto-disabled the moment its rolling edge stops beating the naive
                  baseline
                </span>
