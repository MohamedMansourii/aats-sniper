...
/* --------------------------------- palette -------------------------------- */
/* Recharts renders to <canvas/svg> and cannot read CSS vars — hardcode hex. */
const HEX = {
  up: "#22e39a",
  down: "#ff5d5d",
  info: "#4aa3ff",
...
const fmtPctSigned = (n: number) => `${n >= 0 ? "+" : ""}${round1(n).toFixed(1)}%`;
...
function TableSkeleton({ rows, cols }: { rows: number; cols: number }) {
  return (
    <div className="space-y-px p-3.5" aria-busy="true" aria-label="Loading positions">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <StatTile
            ...
            sub={`${winRatePct}% win · ${wins}/${closedCount}`}
                ...
                <Table className="text-[12px]">
                  <TableHeader className="sticky top-0 z-10 bg-panel">
                    <TableRow className="border-b border-border hover:bg-transparent">
                      ...
                      <TableHead className={TH_R}>Realized PnL</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {closed.map((p) => {
                      ...
                      const roiPct =
                        p.entrySol > 0 ? round1((pnl / p.entrySol) * 100) : 0;
                      return (
                        <TableRow key={p.mint} className={ROW}>
                          <TableCell className={TD}>
                            <TokenCell token={p.token} mint={p.mint} />
                          ...
                          <TableCell className={cn(TD, "text-right")}>
                            <div className="flex flex-col items-end leading-tight">
                              ...
                              <span
                                className={cn("num text-[10px]", pctClass(pnl))}
                              >
                                {fmtPctSigned(roiPct)}
                              </span>
                            </div>
