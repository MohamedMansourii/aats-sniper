    ...
    <div className="space-y-2.5 px-3.5 py-3.5">
      {hops.map(h => {
        ...
        const budgetPct = Math.max(
          0,
          Math.min(100, (h.budgetMs / scaleMax) * 100)
        );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-3 p-3.5">
          ...
          <div className="min-w-0">
            <div className="text-[13px] font-semibold text-text">
              ...
              <span className="num">~{roundInt(slotFloorMs)} ms</span> slot
              floor (irreducible)
            </div>
            <p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">
              The internal pipeline is tunable; the ~400 ms slot floor is
              Solana's block time and is fixed for everyone. Squeezing hops past
              their budget buys you fractions of a slot — never a whole one.
            </p>
          </div>
        </div>
      </Panel>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <Panel title="Infrastructure tier — generic vs colo" icon={<Layers />}>
          {loading ? (
        ...
        <Panel
          ...
          actions={
            colo ? (
              <Chip tone="accent">
                <span className="num">{round1(colo.medianSlotDelay)}</span>
                &nbsp;median
              </Chip>
            ) : undefined
          }
        ...
        <div className="grid grid-cols-1 gap-3.5 p-3.5 md:grid-cols-3">
          <div className="md:col-span-2">
            <p className="text-[12px] leading-relaxed text-muted-foreground">
              A deployer who bundles their own buy into the{" "}
              <span className="text-text">same transaction as the mint</span>{" "}
              lands at <span className="num text-violet">N+0</span> — the
              literal first slot the token exists. No amount of colo, kernel
              bypass, or Jito tipping puts us ahead of a fill that is in the
              creation transaction itself. Our edge is not raw speed; it is the
              gate and the model deciding{" "}
              <span className="text-text">which</span> N+1 entries are worth
              taking and which are the insider's exit liquidity.
            </p>
            <ul className="mt-3 space-y-1.5 text-[12px] text-muted-foreground">
              <li className="flex items-start gap-2">
                ...
                <span>
                  <span className="text-text">N+0 (insider):</span> buy is in
                  the mint tx — unreachable by anyone outside the bundle.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-brand" />
                <span>
                  <span className="text-text">N+1 (us, colo):</span> next-slot
                  landing — the best a public sniper can structurally achieve.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-faint" />
                <span>
                  <span className="text-text">N+2.. (generic):</span> degraded
                  land rate and higher entry slip — where most retail bots
                  actually sit.
                </span>
              </li>
            </ul>
          ...
          <div className="rounded-[8px] border border-border bg-panel2 p-3">
            ...
            <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">
              Treat any "0 ms to land" claim as marketing. Against an in-mint
              cobundle the only winning move is not to race it — it is to grade
              it.
            </p>
